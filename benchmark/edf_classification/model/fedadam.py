...
class Classifier(FModule):
    def __init__(self):
        ...
        self.n_leads = 5
        # self.ln1 = nn.Linear(128*self.n_leads, 128, True)
...
class Model(FModule):
    def __init__(self):
        ...
        for i in range(self.n_leads):
            ...
            self.feature_extractors[i].m = None
            self.feature_extractors[i].v = None
        ...
        self.criterion = nn.CrossEntropyLoss(
            weight = torch.tensor([1.0, 1.0, 0.5, 1.5, 1.0])
        )


    def forward(self, x, y, leads, contrastive_weight=0): # x: B x 12 x C
