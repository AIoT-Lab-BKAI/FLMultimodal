...
def gen_list_modalities (min_num_modal, max_num_modal, NUM_USER=20):
    ...
    for i in range(min_num_modal, max_num_modal+1):
        list_modals_tuples.append(tuple(random.sample(range(0, num_modality), i))) 
    for i in range(NUM_USER-(max_num_modal-min_num_modal+1)):
        gen = tuple(random.sample(range(0, num_modality), random.randint(min_num_modal, max_num_modal)))
...
def take_least_combi(num_combi_test, ls_combi_train):
    possible_coms = list(combinations(range(num_modality),num_combi_test))
